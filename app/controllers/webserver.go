...
func apiCandleHandler(w http.ResponseWriter, r *http.Request) {
	...
	if volume != "" {
		// Volume data is already included in Candle struct, no additional processing needed
	}
	...
	if macd != "" {
		strPeriod1 := r.URL.Query().Get("macdFastPeriod")
		strPeriod2 := r.URL.Query().Get("macdSlowPeriod")
		strPeriod3 := r.URL.Query().Get("macdSignalPeriod")
